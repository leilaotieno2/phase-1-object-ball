  ...
  function numPointsScored(playerName) {
    ...
    for (let team in game) {
      ...
      for (let player in teamPlayers)
      {
        ...
        const teamPlayers = game[team].players;
        for (let player in teamPlayers) {
        teamNumbers.push(teamPlayers[player].number);
        }
        }
        }
        return teamNumbers;
        ...
        const teamPlayers = game[team].players;
        for (let player in teamPlayers) {
        if (teamPlayers[player].shoe > maxShoeSize) {
        maxShoeSize = teamPlayers[player].shoe;
        playerWithMaxShoeSize = player;
        }
        }
        }
        return game[playerWithMaxShoeSize].rebounds;
        }  